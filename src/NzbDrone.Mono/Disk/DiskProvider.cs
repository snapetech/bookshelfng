...
namespace NzbDrone.Mono.Disk
{
    public class DiskProvider : DiskProviderBase
    {
        ...
        public override long? GetAvailableSpace(string path)
        {
            ...
            Logger.Debug($"path: {path?.ReplaceLineEndings("")}");

            var mount = GetMount(path);

            if (mount == null)
            {
                Logger.Debug("Unable to get free space for '{0}', unable to find suitable drive", path?.ReplaceLineEndings(""));
                return null;
            }
                ...
                if (exists && dstInfo.Length == srcInfo.Length)
                {
                    ...
                    if (checkLength > 0)
                    {
                        ...
                        using (var dstStream = new FileStream(destination, FileMode.Open, FileAccess.Read))
                        {
                            ...
                            srcStream.ReadExactly(srcData);
                            dstStream.ReadExactly(dstData);
                        }
